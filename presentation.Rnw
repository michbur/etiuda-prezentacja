...
Peptydy sygnałowe zarodźców malarii zachowują swoją funkcję mając skład aminokwasowy różny od peptydów sygnałowych innych eukariontów.
...
\item uwzględniają otoczenie danej reszty aminokwasowej.
\end{itemize}
...
QuiPT jest o wiele szybszy niż klasyczne testy permutacyjne i zwraca dokładniejsze p-wartości.
