...
\begin{itemize}
\item to krótkie (15-30 aminokwasów) N-końcowe sekwencje,
...
\begin{frame}{n-gramy}
n-gramy (k-tuple, k-mery) to podsekwencje o długości $n$. Dzięki uwzględnianiu szerszego otoczenia danej reszty aminokwasowej, n-gramy niosą ze sobą więcej informacji niż pojedyncze reszty.
...
1-gramy:
<<echo = FALSE, message=FALSE,warning=FALSE,fig.height=5,results='asis'>>=
seq2ngrams(sample_seq[1, 1L:5, drop = FALSE], 1, aa_v, d = 0) %>% 
  decode_ngrams %>% 
  #paste0(",") %>%
  gsub("_", " - ", x = .) %>% 
  cat(sep = ", ")
...
\item Peptydy zanalizowano przy pomocy spektroskopii fourierowskiej (FTIR).
\end{enumerate}
\end{frame}
